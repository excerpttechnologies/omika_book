...
export default function Payment() {
  ...
  // FIXED: loadCartItems function to properly map mrpPrice to salePrice
  const loadCartItems = () => {
    try {
      const savedCart = localStorage.getItem("bookCart");
      if (savedCart) {
        ...
        const validatedCart = parsedCart.map((item: any, index: number) => ({
          ...
          bookName: item.bookName || 'Unknown Book',
          authorName: item.authorName || 'Unknown Author',
          // CRITICAL FIX: Use mrpPrice as salePrice since cart uses mrpPrice
          salePrice: item.salePrice || item.mrpPrice || 0,
          mrpPrice: item.mrpPrice || item.salePrice || 0,
          image1: item.image1 || '/default-book.jpg',
          quantity: item.quantity || 1
        }));
        
        console.log('Validated cart with prices:', validatedCart);
        setCartItems(validatedCart);
      }
    } catch (error) {
      console.error("Error loading cart:", error);
  ...
  // FIXED: calculatePriceDetails function with proper logging
  const calculatePriceDetails = (): PriceDetails => {
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      const priceDetails = calculatePriceDetails();
      console.log('Price details for payment:', priceDetails);
